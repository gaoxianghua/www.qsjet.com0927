	...
	?>
	   ...
	   ?>
        ...
        <div class="lm_list cao_lm_list">
              <?php
					echo "<div style=\"font-size:20px;color:#333;margin-bottom:20px;\">".$bigclass_cn."</div>";
					$pagesize = 9; //分页数量
					if($_REQUEST['page']){
					...
					$rows=mysql_query($sql);
					if($num>0){
						echo "<ul class=\"pro_ul pro_ul3\">";
							while($rs=mysql_fetch_array($rows)){
								echo "<li>";
								echo "<a href=\"tangview.php?newsid=".$rs['id']."\" target=\"_blank\"><img src=\"".$rs['news_pic']."\" width=\"236\" height=\"165\"></a>";
					...
					if($num>$pagesize){
						$url="tangclass.php?classid=$classid";//当前页面
