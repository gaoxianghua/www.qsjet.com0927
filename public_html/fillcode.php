<?php
    ...
    $client_date = chkstr(trim($_REQUEST['client_date']));
	...
	if(strlen($client_name)<2 || strlen($client_pronum)<1 || strlen($passcode)<3 || strlen($client_xinghao)<3){
		echo erro("请将内容填写完整");
	}
	...
	(client_name,client_phone,client_pronum,client_fcode,client_city,client_year,client_yaopin,client_ip,client_xinghao,client_date)
	 values
	 ('$client_name','$client_phone','$client_pronum','$client_fcode','$client_city','$client_year',$client_yaopin,'$client_ip','$client_xinghao','$client_date')";
      ...
      <div class="h_con">
        ...
        <div class="confirm_table">
          ...
          <form action="chkcode.php?action=chk" method="post" name="SearchCode" id="SearchCode">
			...
			<input name="client_date" type="hidden" id="client_date" value="<?=$client_date?>" />
