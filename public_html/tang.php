	...
	?>
	   ...
	   ?>
		<script type="text/javascript">
			$(function(){
				var zong=Math.ceil($(".pro_ul2 li").length/2);
				for(var i=1;i<=zong;i++){
					$(".pro_ul li").eq(2*i-1).css("margin-right",0);
				}
			})
		</script>
		<style type="text/css">
        ...
        <div class="lm_list cao_lm_list">
              <?php
					...
					$numsql="select * from kboy_bigclass where pid=$classid order by bigclass_order asc, id asc";
					...
					$sql="select * from kboy_bigclass where pid=$classid order by bigclass_order asc, id asc limit $page $pagesize";
					$rows=mysql_query($sql);
					if($num>0){
						echo "<ul class=\"pro_ul pro_ul2\">";
							while($rs=mysql_fetch_array($rows)){
								echo "<li>";
								echo "<a href=\"tangclass.php?classid=".$rs['id']."\" target=\"_blank\"><img src=\"".$rs['bigclass_pic']."\" width=\"347\" height=\"238\"></a>";
								echo "<p>".$rs['bigclass_cn']."</p>";
								echo "</li>";				
							}
						echo "</ul>";
					}else{
					...
					if($num>$pagesize){
						$url="tang.php?classid=$classid";//当前页面
