<?php
	...
	$edit_sql = "select * from kboy_news where id=$id";
	...
	if($edit_nums > 0){
		$edit_rs = mysql_fetch_array($edit_rows);
			...
			$news_secclass = $edit_rs['news_secclass'];
			$news_fangan = $edit_rs['news_fangan'];
			...
			$news_hit = $edit_rs['news_hit'];
			$news_secclass = $edit_rs['news_secclass'];
			$news_addtime = $edit_rs['news_addtime'];
	}else{
...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td><strong>后台管理</strong>&nbsp;-&nbsp;<span class="hong"><strong>文章修改</strong></span></td>
  </tr>
...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#333333">
  <form id="myform" name="myform" method="post" action="newsaddsave.php?action=edit">
    ...
    <tr>
      <td width="80" align="right" bgcolor="#EFEFEF">文章标题：</td>
...
<input name="re_url" type="hidden" id="re_url" value="<?=$_SERVER['HTTP_REFERER']?>" /></td>
    ...
    <tr>
      <td align="right" bgcolor="#EFEFEF">文章来源：</td>
      ...
      <td align="right" bgcolor="#EFEFEF">文章作者：</td>
    ...
    <tr>
      <td align="right" bgcolor="#EFEFEF">文章类别：</td>
      <td colspan="5" bgcolor="#EFEFEF">
	  <select name="news_bigclass" id="news_bigclass">
		<option value="">==请选择类别==</option>
		<?php
			$bigIDs = 1;
			getSoonClass_option($bigIDs,'',$news_bigclass,1); // 下拉option
		?>
      </select>	  </td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="right" bgcolor="#EFEFEF">文章简述：</td>
      <td colspan="5" bgcolor="#EFEFEF"><textarea name="news_text" cols="70" rows="4" id="news_text"><?=$news_text?></textarea></td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="right" bgcolor="#EFEFEF">信息日期：</td>
      <td colspan="5" bgcolor="#EFEFEF"><input readonly="" name="news_addtime" type="text" id="news_addtime" value="<?=$news_addtime?>" size="25" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"  /></td>
    </tr>
    <tr>
      <td align="right" bgcolor="#EFEFEF">文章内容：</td>
    ...
    <tr>
      <td align="right" bgcolor="#EFEFEF">视频内容：</td>
