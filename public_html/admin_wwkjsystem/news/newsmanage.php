...
<table width="97%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
  <tr bgcolor="#336699">
    ...
    <td align="center"><span class="bai12">文章标题</span></td>
    <td align="center"><span class="bai12">文章类别</span></td>
  ...
  <?php
		...
		if($SQL != ""){
			$numsql = "select * from kboy_news where ".$SQL." and news_bigclass in(".$getSoonID.")";
		}else{
			$numsql = "select * from kboy_news where news_bigclass in(".$getSoonID.")";
		}
		...
		if($SQL != ""){
			$SQL = "select * from kboy_news where ".$SQL." and news_bigclass in(".$getSoonID.") order by news_top desc,news_order desc, id desc limit $page $pagesize";
		}else{
			$SQL="select * from kboy_news where news_bigclass in(".$getSoonID.") order by news_top desc,news_order desc, id desc limit $page $pagesize";
		}
		$rows = mysql_query($SQL);
		while($rs = mysql_fetch_array($rows)){
		$i=$i+1;
?>
  <form action="newsaddsave.php?action=del" method="post" name="myform" id="myform">
    <tr bgcolor="#efefef" onmouseover="this.style.background='#C9C9C9'" onmouseout="this.style.background='#efefef'">
      ...
      <td align="center" class="lv12"><?= $rs['news_writer']?></td>
