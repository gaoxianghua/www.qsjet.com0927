...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td><strong>后台管理</strong>&nbsp;-&nbsp;<span class="hong"><strong>防伪码查询</strong></span></td>
  </tr>
</table>
...
<table width="97%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
  <tr bgcolor="#336699">
    ...
    <td align="center"><span class="bai12">用户姓名</span></td>
    <td align="center"><span class="bai12">联系电话</span></td>
    <td align="center"><span class="bai12">产品编号</span></td>
    <td align="center"><span class="bai12">防伪编码</span></td>
    <td align="center"><span class="bai12">联系地址</span></td>
    <td align="center"><span class="bai12">注射治疗史</span></td>
    <td align="center"><span class="bai12">皮下有无硬结</span></td>
    <td align="center"><span class="bai12">购买日期</span></td>
    <td align="center"><span class="bai12">查询结果</span></td>
    <td align="center"><span class="bai12">操作IP</span></td>
    <td align="center"><span class="bai12">查询日期</span></td>
  </tr>
  <?php
		$sql="";
		...
		if($keys != ""){
			if($sql != ""){
				$sql = $sql." and (client_name like '%".$keys."%' or client_phone like '%".$keys."%' or client_pronum  like '%".$keys."%' or client_fcode like '%".$keys."%')";
			}else{
				$sql = " (client_name like '%".$keys."%' or client_phone like '%".$keys."%' or client_pronum  like '%".$keys."%' or client_fcode like '%".$keys."%')";
			}
		}
...
if($sql != ""){
	$numsql="select * from kboy_message where ".$sql." order by id desc";
}else{
	$numsql="select * from kboy_message";
}
...
if($sql!=""){
$sql="select * from kboy_message where ".$sql." order by id desc limit $page $pagesize";
}else{
$sql="select * from kboy_message order by id desc limit $page $pagesize";
}
...
?>
  <form action="newsaddsave.php?action=del" method="post" name="myform" id="myform">
    <tr bgcolor="#efefef" onmouseover="this.style.background='#C9C9C9'" onmouseout="this.style.background='#efefef'">
      ...
      <td align="center"><?=$rs['client_phone']?></td>
      <td align="center"><?=$rs['client_pronum']?></td>
      <td align="center"><?=$rs['client_fcode']?></td>
      <td align="center"><?=$rs['client_city']?></td>
      <td align="center"><?=$rs['client_year']?></td>
      <td align="center"><?=getClassName('kboy_bigclass','bigclass_cn','id',$rs['client_yaopin'],'NUM')?></td>
      <td align="center"><?=$rs['client_date']?></td>
      <td align="center"><?=$rs['client_result']?></td>
      <td align="center"><?=$rs['client_ip']?></td>
      <td align="center"><?= $rs['client_addtime']?></td>
    </tr>
	<?php
	}if($num>0){
	?>
