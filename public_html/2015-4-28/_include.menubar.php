				...
				if($bigclass_nums >0){
					while($bigclass_rs = mysql_fetch_array($bigclass_rows)){
						...
						echo "<a href=\"".$bigclass_rs['bigclass_urls']."\">".$bigclass_rs['bigclass_cn']."</a>";
							$secclass_sql = "select * from kboy_bigclass where pid=".$bigclass_rs['id']." order by bigclass_order asc, id asc";
								...
								while($secclass_rs = mysql_fetch_array($secclass_rows)){
									...
									echo "<a href=\"".$secclass_rs['bigclass_urls']."\">".$secclass_rs['bigclass_cn']."</a>";
										if($bigclass_rs['id'] == 64 || $bigclass_rs['id'] == 68){
											$thrclass_sql = "select * from kboy_news where news_bigclass=".$secclass_rs['id']." order by news_top desc,news_order desc, id desc";
											...
											if($thrclass_nums >0){
												...
												while($thrclass_rs = mysql_fetch_array($thrclass_rows)){
													echo "<li><a href=\"newsview.php?newsid=".$thrclass_rs['id']."\">".$thrclass_rs['news_title']."</a>";
												}
												echo "</ul>";
											}
										}
