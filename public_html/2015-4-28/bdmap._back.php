 ...
 // 百度地图API功能
	...
	var data_info = [<?php
			...
			if($map_nums >0){
				$i = 1;
				while($map_rs = mysql_fetch_array($map_rows)){
					echo "[".$map_rs['news_x'].",".$map_rs['news_y'].",\"".$map_rs['news_address']."<br>网点类型：".getClassName('kboy_bigclass','bigclass_cn','id',$map_rs['news_bigclass'],'NUM')."\",\"".$map_rs['news_phone']."\",\"".$map_rs['news_title']."\"]";
					if($i < $map_nums){
						echo ",";
					}
				$i ++ ;	
