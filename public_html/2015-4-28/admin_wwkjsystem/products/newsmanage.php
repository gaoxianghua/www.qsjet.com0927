...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td><strong>后台管理</strong>&nbsp;-&nbsp;<span class="hong"><strong>产品管理</strong></span></td>
  </tr>
</table>
...
<table width="97%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
  <tr bgcolor="#336699">
    ...
    <td width="50" align="center"><span class="bai12">转页</span></td>
    <td width="100" align="center"><span class="bai12">语言版本</span></td>
    <td width="82" align="center"><span class="bai12">产品图片</span></td>
    <td align="center"><span class="bai12">产品标题</span></td>
    <td width="160" align="center"><span class="bai12">产品类别</span></td>
    <td width="100" align="center"><span class="bai12">发布作者</span></td>
  ...
  <?php
		...
		if($SQL != ""){
			$numsql = "select * from kboy_pro where ".$SQL." and news_bigclass in(".$getSoonID.")";
		}else{
			$numsql = "select * from kboy_pro where news_bigclass in(".$getSoonID.")";
		}
		...
		if($SQL != ""){
			$SQL = "select * from kboy_pro where ".$SQL." and news_bigclass in(".$getSoonID.") order by news_top desc,news_order desc, id desc limit $page $pagesize";
		}else{
			$SQL="select * from kboy_pro where news_bigclass in(".$getSoonID.") order by news_top desc,news_order desc, id desc limit $page $pagesize";
		}
		$rows = mysql_query($SQL);
		while($rs = mysql_fetch_array($rows)){
		$i=$i+1;
?>
  <form action="newsaddsave.php?action=del" method="post" name="myform" id="myform">
    <tr bgcolor="#efefef" onmouseover="this.style.background='#C9C9C9'" onmouseout="this.style.background='#efefef'">
      ...
      <td align="center">
	  <?php
	  	if(strlen($rs['news_tourls'])>2){
			echo "<span class=\"hong\">是</span>";
		}else{
			echo "否";
		}
	  ?>	  </td>
      ...
      <td align="center"><?= $rs['news_writer']?></td>
	  ...
	  <?php
			if($num > $pagesize){
				if ($pageval<=1){
					...
					echo "共&nbsp;<font color='red'>".$num."</font>&nbsp;条产品&nbsp;&nbsp;";
