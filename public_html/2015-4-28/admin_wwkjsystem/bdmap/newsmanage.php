...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td><strong>后台管理</strong>&nbsp;-&nbsp;<span class="hong"><strong>网点管理</strong></span></td>
  </tr>
</table>
...
<table width="97%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000">
  <tr bgcolor="#336699">
    ...
    <td align="center"><span class="bai12">网点名称</span></td>
    <td align="center"><span class="bai12">所属类别</span></td>
    <td align="center"><span class="bai12">联系电话</span></td>
    <td align="center"><span class="bai12">详细地址</span></td>
    <td width="150" align="center"><span class="bai12">添加日期</span></td>
  ...
  <?php
		...
		if($SQL != ""){
			$numsql = "select * from kboy_bdmap where ".$SQL;
		}else{
			$numsql = "select * from kboy_bdmap";
		}
		...
		if($SQL != ""){
			$SQL = "select * from kboy_bdmap where ".$SQL." order by id desc limit $page $pagesize";
		}else{
			$SQL="select * from kboy_bdmap order by id desc limit $page $pagesize";
		}
		$rows = mysql_query($SQL);
		while($rs = mysql_fetch_array($rows)){
		$i=$i+1;
?>
  <form action="newsaddsave.php?action=del" method="post" name="myform" id="myform">
    <tr bgcolor="#efefef" onmouseover="this.style.background='#C9C9C9'" onmouseout="this.style.background='#efefef'">
      ...
      <td align="center"><?=$rs['news_phone']?></td>
      <td align="center"><?=$rs['news_address']?></td>
      <td align="center"><?= $rs['news_addtime']?></td>
      <td align="center">
	  <a href="newsedit.php?id=<?= $rs['id'] ?>">修改</a></td>
    </tr>
    <?php
		}if($num > 0){
	?>
