<?php
	...
	$edit_sql = "select * from kboy_bdmap where id=$id";
	...
	if($edit_nums > 0){
		$edit_rs = mysql_fetch_array($edit_rows);
			...
			$news_phone = $edit_rs['news_phone'];
			$news_address = $edit_rs['news_address'];
			$news_x = $edit_rs['news_x'];
			$news_y = $edit_rs['news_y'];
...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="0">
  <tr>
    <td><strong>后台管理</strong>&nbsp;-&nbsp;<span class="hong"><strong>网点修改</strong></span></td>
  </tr>
...
<table width="97%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#333333">
  <form id="myform" name="myform" method="post" action="newsaddsave.php?action=edit">
    ...
    <tr>
      ...
      <td bgcolor="#EFEFEF"><input name="news_title" type="text" id="news_title" value="<?=$news_title?>" size="50" />
    ...
    <tr>
      ...
      <td bgcolor="#EFEFEF"><input name="news_phone" type="text" id="news_phone" value="<?=$news_phone?>" size="30" />
        <input name="news_x" type="hidden" id="news_x" value="<?=$news_x?>" />
        <input name="news_y" type="hidden" id="news_y" value="<?=$news_y?>" /></td>
    </tr>
    <tr>
...
<script type="text/javascript">
	// 百度地图API功能
		...
		map.centerAndZoom(new BMap.Point(<?=$news_x?>,<?=$news_y?>), 15);
		
		<?php
				if(strlen($news_x)>0){
					echo "var marker = new BMap.Marker(new BMap.Point(".$news_x.",".$news_y."));";
