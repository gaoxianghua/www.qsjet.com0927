<?php
	...
	if($action == "add"){
		...
		$news_sql = "
		insert into kboy_piclink
		(news_language,news_title,news_bigclass,news_pic,news_tourls,news_text)
		values
		($news_language,'$news_title',$news_bigclass,'$news_pic','$news_tourls','$news_text')
		";
		...
		if($res){
			...
			mysql_query("update kboy_piclink set news_order='$get_id' where id=$get_id");	
		...
		$news_sql = "update kboy_piclink set
			news_language=$news_language,
			news_title='$news_title',
			news_bigclass=$news_bigclass,
			news_pic='$news_pic',
			news_text='$news_text',
			news_tourls='$news_tourls' where id=$id";
		$res = mysql_query($news_sql);
		if($res){
			echo ok("修改成功",$re_url);
		}else{
		...
		if($news_option == "del"){
			if(strlen($newsid)>0){
				$news_sql = "delete from kboy_piclink where id in($newsid)";
				$res = mysql_query($news_sql);
				if($res){
		...
		$news_sql = "update kboy_piclink set news_top='$now_time' where id=$id";
		$res = mysql_query($news_sql);
		if($res){
		...
		$news_sql = "update kboy_piclink set news_top='' where id=$id";
		$res = mysql_query($news_sql);
		if($res){
			echo re($_SERVER['HTTP_REFERER']);
		}else{
		...
		$news_sql = "update kboy_piclink set news_order=$news_order where id=$id"; 
		$res = mysql_query($news_sql);
		if($res){
			echo re($_SERVER['HTTP_REFERER']);
		}else{
