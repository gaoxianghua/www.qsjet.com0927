...
<div class="product clear">
  ...
  <div class="more"><a href="products.php?classid=135">+MORE</a></div>
  <ul class="product_show">
  
	  	<?php
				$PIDs = getSoonID(134,$styleID);
				$PIDs = ltrim($getSoonID,',');
				if(strlen($PIDs)>0){
					$news_sql = "select * from kboy_pro where news_bigclass in(".$PIDs.") order by news_top desc,news_order desc, id desc limit 5";
					$news_rows = mysql_query($news_sql);
					$news_nums = mysql_num_rows($news_rows);
					if($news_nums >0){
						while($news_rs = mysql_fetch_array($news_rows)){
							echo "<li><img src=\"".$news_rs['news_pic']."\" alt=\"".$news_rs['news_title']."\" /><a href=\"proview.php?newsid=".$news_rs['id']."\" target=\"_blank\"><p><b>".$news_rs['news_title']."</b></p></a></li>";
						}
					}
				}
		?>
  ...
  <dl>
    <dd>
      ...
      <div style=" width:455px; height:365px; border:1px solid #999;"><h3 style=" font-size:30px; line-height:300px; text-align:center;">
		...
		var so = new SWFObject("/CuPlayer/CuPlayerMiniV4.swf","CuPlayerV4","455","365","9","#000000");
		so.addParam("allowfullscreen","true");
		so.addParam("allowscriptaccess","always");
		so.addParam("wmode","opaque");
		so.addParam("quality","high");
		so.addParam("salign","lt");
		so.addVariable("CuPlayerSetFile","/CuPlayer/CuPlayerSetFile.php"); //播放器配置文件地址
		so.addVariable("CuPlayerFile","<?=$index_mk6?>"); //视频文件地址
    ...
    <dd>
      <div class="video_title"><b>新闻快递</b>/NEWS</div>
      <ul class="academic">
	  
	  	<?php
				$news_sql = "select * from kboy_news where news_bigclass=99 order by news_top desc,news_order desc, id desc limit 6";
				$news_rows = mysql_query($news_sql);
				$news_nums = mysql_num_rows($news_rows);
				if($news_nums >0){
					while($news_rs = mysql_fetch_array($news_rows)){
				...
				if($ad_nums >0){
					while($ad_rs = mysql_fetch_array($ad_rows)){
						if($ad_rs['news_tourls']){
							echo "<a href=\"".$rs['news_tourls']."\" target=\"_blank\"><img src=\"".$ad_rs['news_pic']."\" border=\"0\" alt=\"".$news_rs['news_title']."\" /></a>";
						}else{
...
<div class="experience clear clearfix">
  <ul class="exper_center">
    <li><a id="tiyan_01" href="news.php?classid=132"></a></li>
